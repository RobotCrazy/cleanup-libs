...
float Adafruit_VEML7700::getResolution(void)
{
    ...
    // Converted getIntegrationTimeValue() to float
}
...
float Adafruit_VEML7700::autoLux(void)
{
    ...
    if (ALS <= ((uint16_t)100)) // Typecasted 100 to uint16_t type
    {

        // increase first gain and then integration time as needed
        // compute lux using simple linear formula
        while (((ALS <= ((uint16_t)100))) && !((gainIndex == ((uint8_t)(3))) && (itIndex == ((uint8_t)(5)))))
        // Typecasted 100 to uint16_t ; typecasted 3 and 5 to uint8_t
        {
            if (gainIndex < ((uint8_t)(3))) // Typecasted 3 to uint8_t
            {
                setGain(gains[++gainIndex]);
            }
            else if (itIndex < ((uint8_t)(5))) // Typecasted 5 to uint8_t
            {
                setIntegrationTime(intTimes[++itIndex]);
            }
            ALS = readALS(true);
